...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		if (isset($_POST['userName']) ) {
			if(empty($_POST['userName'])) {
				...
				$name = "<b>Імя: </b>" . strip_tags($_POST['userName']) . "; ";
			}
		}
        ...
        if (isset($_POST['agreement']) ) {
            ...
            if(empty($_POST['agreement'])) {
                ...
                $agreement = "<b>Підтвердження: </b>" . strip_tags($_POST['agreement']) . "<br>";
            }
        }
		...
		foreach ($_POST as $key =>  $value) {
			...
			if (is_array($value) || $value instanceof Traversable) {
				foreach ($value as $k => $v) {
					...
					if($k == 'productImg'){
						$productImg = '<img src="' . $v . '" width="100" height="100" alt="картинка товару">';
						$body.=
							'
					...
					if($k == 'productSize'){
						if(!empty($v)){
							...
							$body.=
								'<td style="width: 100px;  padding-top:15px; padding-bottom:15px; padding-right:15px; padding-left:15px; text-align:center; border-top:1px; border-left:1px; border-right:0; border-bottom:0; border-color:#e2e2e2; border-style: solid;" >
									<div style="padding: 5px;"> Розмір відсутній </div>
								</td>';
						}
					}
					if($k == 'productId'){
					...
					if($k == 'productQuantity'){
						$body.=
							'<td style="width: 100px;  padding-top:15px; padding-bottom:15px; padding-right:15px; padding-left:15px; text-align:center; border-top:1px; border-left:1px; border-right:0; border-bottom:0; border-color:#e2e2e2; border-style: solid;" >
												<div style="padding: 5px;"> Кількість: 
												'
							. $v .
							'
												</div></td>';
					...
					if($k == 'productPriceCommon'){
						$body.=
							'<td style="width: 100px;  padding-top:15px; padding-bottom:15px; padding-right:15px; padding-left:15px; text-align:center; border-top:1px; border-left:1px; border-right:0; border-bottom:0; border-color:#e2e2e2; border-style: solid;" >
												<div style="padding: 5px;"> Загальна ціна:
												'
							. $v .
							'
												</div></td>';
    ...
    $mail->isHTML(true);
		$mail->Subject = SUBJECT; // Заголовок листа
